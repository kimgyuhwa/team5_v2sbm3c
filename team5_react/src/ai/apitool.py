...
def chat_tool(input: dict) -> str:
    ...
    needs_rag = any(word in message for word in ["견적", "프로필", "전문가"])
    if needs_rag:
        final_answer = query_engine.query(message).response
    else:
    ...
    if is_valid_summary(key_info) and userno:
        try:
            requests.post("http://localhost:9093/chatbot/api/save", json={"userno": userno, "content": key_info})
...
# ✅ agent에 도구 등록
tools = [
    Tool(name="질문응답", func=lambda input: chat_tool({"input": input, "userno": CURRENT_USERNO}), description="일반 질문 + 중요정보 저장 + RAG"),
    Tool(name="번역", func=translate_tool, description="문장을 외국어로 번역"),
    Tool(name="요약", func=summarize_tool, description="글 요약")
]

agent = initialize_agent(tools=tools, llm=llm, agent_type="openai-functions", verbose=True, max_iterations=2)
