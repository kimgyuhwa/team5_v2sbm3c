    ...
    <div className="max-w-screen-lg mx-auto px-4 py-6">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 bg-white rounded-xl shadow-sm p-6 min-h-[400px] max-h-[80vh] overflow-y-auto">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">예약 목록</h1>
            <p className="text-gray-600">예약은 여기서 확인 할 수 있어요!</p>
          </div>
            ...
            {Object.keys(groupedReservations).length > 0 ? (
              Object.entries(groupedReservations).map(([date, reservations]) => (
                <div key={date} className="space-y-4">
                  <div className="text-left text-lg font-semibold text-gray-700 border-b border-gray-200 pb-2">{date}</div>
                  <div className="space-y-4 ml-4">
                    {reservations.map((reservation) => (
                      ...
                      >
                        <div className="p-4 cursor-pointer" onClick={() => toggleExpanded(reservation.reservationno)}>
                              ...
                              <div className="space-y-3">
                                <div className="flex items-center">
                                  ...
                                  <span className="font-medium">
                                    {loginUser.schoolname} {reservation.schoolgwanname} {reservation.placename}
                                  </span>
                                </div>
                                <div className="flex items-center">
                                  <Clock className="w-4 h-4 mr-1" />
                                  <span className="ml-1 font-medium">
                                    신청일: {reservation.createdAt.split('T')[0]} {reservation.createdAt.split('T')[1].slice(0, 5)}
                                  </span>
                                </div>
                              </div>
                              <div>
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-600 mb-2">예약이 없습니다</h3>
                <p className="text-gray-500">검색 조건을 변경하거나 새로운 예약을 추가해보세요.</p>
              </div>
            )}
          </div>
        </div>
